...
public class HelicopterScr : MonoBehaviour
{
    ...
    public void RoamAround()
    {
        ...
        if (NeedNewPosition)
        {
            ...
            if (DriveMRight)
            {
                M = new Vector3(S.x + Vector3.Distance(gameObject.transform.position, RandomPOsition) / 2, gameObject.transform.position.y, Vector3.Distance(gameObject.transform.position, RandomPOsition) * 1.15473441108545f * -1);
            }
            else
            {
                M = new Vector3(S.x - Vector3.Distance(gameObject.transform.position, RandomPOsition) / 2, gameObject.transform.position.y, Vector3.Distance(gameObject.transform.position, RandomPOsition) * 1.15473441108545f * 1);
